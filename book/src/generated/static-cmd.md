...
| `fold` | Fold the selection if it spans multiple lines, otherwise fold text objects and syntax regions at the cursor | normal: `` Zf ``, `` zf ``, select: `` Zf ``, `` zf `` |
| `unfold` | Unfold the innermost fold at the cursor, or all folds overlapping the selection | normal: `` ZF ``, `` zF ``, select: `` ZF ``, `` zF `` |
| `toggle_fold` | Toggle fold for the text object or syntax region at the primary cursor | normal: `` Z<A-f> ``, `` z<A-f> ``, select: `` Z<A-f> ``, `` z<A-f> `` |
