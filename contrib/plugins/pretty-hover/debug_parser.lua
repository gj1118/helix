...
local function parse_line(line, config, state)
    ...
    if trimmed:match("^[%*]+%s*@") or (state.current_group and trimmed:match("^[%*]+")) then
        clean_line = trimmed:gsub("^[%*]+%s*", "")
...
print("TEST 1: Brief")
print(parse_line(" * @brief Calculates the sum", config, state))

print("\nTEST 2: Param")
print(parse_line(" * @param a The first number", config, state))
